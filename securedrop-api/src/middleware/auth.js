...
const verifyToken = (req, res, next) => {
  ...
  const publicIssuer = `https://keycloak.securedrop.gr/realms/${config.keycloak.realm}`;
  ...
  jwt.verify(
    ...
    (err, decoded) => {
      ...
      if (
        decoded.iss !== internalIssuer &&
        decoded.iss !== externalIssuer &&
        decoded.iss !== publicIssuer
      ) {
