...
wait_for_scale_to_zero() {
  echo "Waiting for scanner scale-to-zero..."
  ...
  while true; do
    ...
    if [ "$running_count" -eq 0 ]; then
      echo "Scanner is at zero pods."
      return 0
    fi

    now=$(date +%s)
    if [ $((now - start)) -gt "$WAIT_ZERO_TIMEOUT" ]; then
      echo "ERROR: Timeout waiting for scanner to scale to zero."
      kubectl get pods -n "$NAMESPACE" -l "$SCANNER_LABEL" || true
      return 1
    fi

    echo "Scanner still running pods=$running_count. Waiting..."
    sleep 5
  done
}
...
# Kubernetes timestamps may have nanoseconds; Python supports microseconds.
if "." in s:
    head, tail = s.split(".", 1)
    if "+" in tail:
...
PY
  ...
  echo "Waiting a bit before next cold run..."
  sleep 5
done
